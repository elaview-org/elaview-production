...
public interface ICampaignRepository {
    ...
    Task<IReadOnlyList<Booking>> GetBookingsByCampaignIdAsync(Guid campaignId, CancellationToken ct);
}
...
public sealed class CampaignRepository(
    ...
    public async Task<IReadOnlyList<Booking>> GetBookingsByCampaignIdAsync(Guid campaignId, CancellationToken ct) =>
        await bookingsByCampaignId.LoadAsync(campaignId, ct) ?? [];
}

internal static class CampaignDataLoaders {
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, Campaign>> GetCampaignById(
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, AdvertiserProfile>> GetAdvertiserByCampaignId(
        IReadOnlyList<Guid> campaignIds, AppDbContext context, CancellationToken ct
    ) => await context.Campaigns
