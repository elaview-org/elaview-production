...
internal static class ReviewDataLoaders {
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, Booking>> GetBookingByReviewId(
        IReadOnlyList<Guid> reviewIds, AppDbContext context, CancellationToken ct
    ) => await context.Reviews
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, Space>> GetSpaceByReviewId(
        IReadOnlyList<Guid> reviewIds, AppDbContext context, CancellationToken ct
    ) => await context.Reviews
        .Where(r => reviewIds.Contains(r.Id))
        .Include(r => r.Space)
