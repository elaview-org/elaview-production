...
public interface IBookingRepository {
    ...
    Task<IReadOnlyList<Review>> GetReviewsByBookingIdAsync(Guid bookingId, CancellationToken ct);
    Task<IReadOnlyList<Payment>> GetPaymentsByBookingIdAsync(Guid bookingId, CancellationToken ct);
    Task<IReadOnlyList<Payout>> GetPayoutsByBookingIdAsync(Guid bookingId, CancellationToken ct);
}
...
public sealed class BookingRepository(
    ...
    public async Task<IReadOnlyList<Review>> GetReviewsByBookingIdAsync(Guid bookingId, CancellationToken ct) =>
        await reviewsByBookingId.LoadAsync(bookingId, ct) ?? [];

    public async Task<IReadOnlyList<Payment>> GetPaymentsByBookingIdAsync(Guid bookingId, CancellationToken ct) =>
        await paymentsByBookingId.LoadAsync(bookingId, ct) ?? [];

    public async Task<IReadOnlyList<Payout>> GetPayoutsByBookingIdAsync(Guid bookingId, CancellationToken ct) =>
        await payoutsByBookingId.LoadAsync(bookingId, ct) ?? [];
}
...
internal static class BookingDataLoaders {
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, Campaign>> GetCampaignByBookingId(
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, Space>> GetSpaceByBookingId(
        IReadOnlyList<Guid> bookingIds, AppDbContext context, CancellationToken ct
    ) => await context.Bookings
        .Where(b => bookingIds.Contains(b.Id))
        .Include(b => b.Space)
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, BookingProof>> GetProofByBookingId(
        IReadOnlyList<Guid> bookingIds, AppDbContext context, CancellationToken ct
    ...
    [DataLoader]
    public static async Task<IReadOnlyDictionary<Guid, BookingDispute>> GetDisputeByBookingId(
        IReadOnlyList<Guid> bookingIds, AppDbContext context, CancellationToken ct
    ) => await context.BookingDisputes
