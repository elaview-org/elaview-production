    ...
    <div className="flex-1 overflow-hidden p-6 space-y-6 flex flex-col">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-orange-700 bg-orange-900/20 p-4">
          ...
          <p className="text-2xl font-bold text-orange-400">{stats?.revisitDue || 0}</p>
        </div>
      ...
      <div className="flex-1 overflow-auto rounded-lg border border-slate-700 bg-slate-800">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="text-lg font-medium text-white">No research entries</p>
          </div>
        ) : (
          <table className="w-full">
            <thead className="bg-slate-900/50 sticky top-0">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Scale</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Why Not Now</th>
            ...
            <tbody className="divide-y divide-slate-700">
              {research.map((r) => (
                <tr key={r.id} className="hover:bg-slate-700/50">
                  <td className="px-4 py-2 text-sm text-white">{r.companyName}</td>
                  <td className="px-4 py-2 text-sm text-slate-300">{r.businessType}</td>
                  <td className="px-4 py-2 text-sm text-slate-300">{r.estimatedScale.replace('_', ' ')}</td>
                  <td className="px-4 py-2 text-sm text-slate-400">{r.reasonNotPursuing || '-'}</td>
