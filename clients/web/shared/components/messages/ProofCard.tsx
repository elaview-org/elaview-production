...
export function ProofCard({ message, booking, isAdvertiser, isOwnMessage }: ProofCardProps) {
  ...
  const isDisputed = message.proofStatus === "DISPUTED" || message.proofStatus === "UNDER_REVIEW";
  ...
  const proofUploadDate = booking.proofUploadedAt ? new Date(booking.proofUploadedAt) : null;
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const autoApprovalTime = new Date(uploadTime.getTime() + 48 * 60 * 60 * 1000);
  ...
  return (
    <>
      <div className={`max-w-2xl rounded-xl border-2 shadow-xl ${
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium border ${
            ...
            isDisputed ? "bg-red-500/10 text-red-400 border-red-500/20" :
            "bg-slate-500/10 text-slate-400 border-slate-500/20"
          }`}>
            {isPending && <Clock className="h-3 w-3" />}
            {isApproved && (wasAutoApproved ? <Zap className="h-3 w-3" /> : <CheckCircle className="h-3 w-3" />)}
        ...
        {message.content && message.content !== "📸 Installation proof submitted" && (
          ...
          <div className="space-y-3">
              ...
              <div className={`pt-3 border-t ${
                ...
                <div className="flex items-center justify-between">
                  <p className={`text-sm font-medium flex items-center gap-2 ${
                    ...
                    <Clock className={`h-4 w-4 ${urgency === 'critical' ? 'animate-pulse' : ''}`} />
                    {timeRemaining}
                  </p>
                  {urgency === 'critical' && (
                    <span className="text-xs text-red-400 animate-pulse">⚠️ Review soon!</span>
                  )}
                </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-semibold text-white">Payment on Approval</h4>
              </div>
              <div className="space-y-1 text-sm">
                <div className="flex justify-between text-slate-300">
                  <span>Installation Fee:</span>
                  <span className="font-mono">${installationFee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-slate-300">
                  <span>First Rental Payout ({booking.totalDays <= 7 ? '70%' : booking.totalDays <= 30 ? '50%' : '40%'}):</span>
                  <span className="font-mono">${firstRentalPayout.toFixed(2)}</span>
                </div>
              ...
              {booking.totalDays > 7 && (
                <p className="text-xs text-slate-400 mt-2">
                  Remaining balance paid in checkpoints {booking.totalDays > 30 ? 'every 30 days' : 'on completion'}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 pt-3 border-t border-yellow-500/20">
              ...
              <p className="text-xs text-center text-slate-400 mt-2">
                Have questions? Email <a href="mailto:support@elaview.com" className="text-blue-400 hover:underline">support@elaview.com</a>
              </p>
            </div>
          </div>
        )}
        ...
        {/* Report Issue Form */}
        {isAdvertiser && isPending && showReportIssueForm && message.bookingId && (
          <ReportIssueForm
            booking={booking}
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-green-300">
                  {wasAutoApproved 
                    ...
                    : `Approved by advertiser on ${format(new Date(message.proofApprovedAt!), "MMM d, h:mm a")}`
                  }
                </p>
                {!isOwnMessage && (
                  <div className="mt-2 bg-green-500/10 rounded-lg p-2 border border-green-500/20">
                    <p className="text-xs font-semibold text-green-400 mb-1">💰 Payment Processed</p>
            ...
            <div className="flex items-start gap-2 bg-red-500/10 rounded-lg p-3 border border-red-500/20">
              ...
              <div>
                ...
                <p className="text-xs text-red-400 mt-1">
                  Our support team is reviewing this installation. You'll receive a decision within 24-48 hours.
                </p>
