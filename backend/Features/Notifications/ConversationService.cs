...
public interface IConversationService {
    ...
    IQueryable<ConversationParticipant> GetParticipantsByConversationIdQuery(Guid conversationId);
    ...
    Task<ConversationParticipant> MarkConversationReadAsync(Guid conversationId, CancellationToken ct);
...
public sealed class ConversationService(
    ...
    public IQueryable<Conversation> GetConversationByIdQuery(Guid id) =>
        context.Conversations.Where(c => c.Id == id);

    public IQueryable<ConversationParticipant> GetParticipantsByConversationIdQuery(Guid conversationId) =>
    ...
    public async Task<Conversation?> GetConversationByIdAsync(Guid id, CancellationToken ct) =>
        await conversationRepository.GetByIdAsync(id, ct);

    public async Task<Conversation> GetOrCreateBookingConversationAsync(Guid bookingId, CancellationToken ct) {
        var existing = await conversationRepository.GetByBookingIdAsync(bookingId, ct);
    ...
    public async Task<ConversationParticipant> MarkConversationReadAsync(Guid conversationId, CancellationToken ct) {
        var userId = GetCurrentUserId();
    ...
    public async Task<int> GetUnreadConversationsCountAsync(CancellationToken ct) {
        var userId = GetCurrentUserId();
