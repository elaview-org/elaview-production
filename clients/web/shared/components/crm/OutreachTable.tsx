...
import { Loader2, ChevronDown, ChevronRight, Mail, Phone, MessageSquare, User, ExternalLink } from "lucide-react";
...
export function OutreachTable() {
  const [channelFilter, setChannelFilter] = useState<OutreachChannel | undefined>();
  ...
  const handleUpdateField = (id: string, field: 'responded' | 'responseDate' | 'responseSummary', value: any) => {
  ...
  const getChannelColor = (channel: OutreachChannel) => {
    switch (channel) {
      ...
      case 'LINKEDIN': return 'text-purple-400';
      case 'WHATSAPP': return 'text-yellow-400';
      case 'IN_PERSON': return 'text-orange-400';
  ...
  return (
    <div className="flex-1 overflow-hidden p-6 space-y-6 flex flex-col">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <select
          value={channelFilter || ''}
          onChange={(e) => setChannelFilter(e.target.value as OutreachChannel || undefined)}
        ...
        <select
          value={respondedFilter === undefined ? '' : respondedFilter.toString()}
          onChange={(e) => setRespondedFilter(e.target.value === '' ? undefined : e.target.value === 'true')}
          className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
        ...
        {(channelFilter || respondedFilter !== undefined || dateFrom || dateTo) && (
          <button
            onClick={() => {
      ...
      <div className="flex-1 overflow-hidden rounded-lg border border-slate-700 bg-slate-800 shadow-sm">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="text-lg font-medium text-white">No outreach logs found</p>
                ...
                {logs.map((log) => (
                  <>
                    ...
                    >
                      ...
                      {/* Channel */}
                      <td className="px-4 py-2 border-r border-slate-700">
                        <div className={`flex items-center gap-2 ${getChannelColor(log.channel)}`}>
                      ...
                      <td className="px-4 py-2 border-r border-slate-700">
                        <input
                          ...
                          onChange={(e) => handleUpdateField(log.id, 'responded', e.target.checked)}
                      ...
                      <td className="px-4 py-2 border-r border-slate-700">
                        <input
                          type="date"
                          value={log.responseDate ? new Date(log.responseDate).toISOString().split('T')[0] : ''}
                          onChange={(e) => handleUpdateField(log.id, 'responseDate', e.target.value ? new Date(e.target.value) : null)}
                          disabled={updateOutreach.isPending}
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          onChange={(e) => handleUpdateField(log.id, 'responseSummary', e.target.value || null)}
                          disabled={updateOutreach.isPending}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
                                  <p className="text-sm text-slate-300 whitespace-pre-wrap">
                                    {log.messageContent || 'No message content recorded'}
                                  </p>
                                </div>
                              </div>
                              {log.sentBy && (
