...
import { Camera, Calendar, CheckCircle, Clock, AlertCircle, Loader2 } from 'lucide-react';
...
export default function ActiveCampaignsPage() {
      ...
      <div className="h-full w-full p-4">
        <div className="h-full bg-slate-900 border border-slate-800 rounded-xl shadow-xl flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-slate-400">Loading active campaigns...</p>
          </div>
        </div>
      </div>
    );
  ...
  const noVerificationNeeded = bookings?.filter(b => !b.nextVerificationDue) ?? [];
      ...
      <div className="h-full bg-slate-900 border border-slate-800 rounded-xl shadow-xl overflow-hidden flex flex-col">
        ...
        <div className="flex-shrink-0 p-6 border-b border-slate-700">
          ...
          <p className="text-slate-400 mt-2">
            {bookings?.length || 0} active campaign{bookings?.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeBookings.length > 0 && (
            <div className="mb-8">
              <h2 className="text-xl font-semibold text-white mb-4">Requires Verification</h2>
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg text-white">{booking.space.title}</h3>
                          <p className="text-sm text-slate-400">{booking.campaign.name}</p>
                        </div>
                        ...
                        {hasVerifications && (
                          <>
                            ...
                            <div className="text-xs text-slate-500">
                              Completed: {schedule.checkpoints.filter((cp: any) => cp.completed).length} / {schedule.checkpoints.length} verifications
                            </div>
                          </>
                        )}
                      </div>
                      ...
                      {hasVerifications && booking.nextVerificationDue && (
                        <Link
                          ...
                          className={`mt-4 w-full py-2.5 rounded-lg flex items-center justify-center font-medium transition-all ${
                            verificationStatus === 'overdue' || verificationStatus === 'due-soon'
          ...
          {noVerificationNeeded.length > 0 && (
            <div>
              <h2 className="text-xl font-semibold text-white mb-4">Running (No Verification Needed)</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {bookings?.length === 0 && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-12 text-center shadow-lg">
              ...
              <p className="text-slate-400">Your active campaigns will appear here</p>
            </div>
          )}
        </div>
      </div>
