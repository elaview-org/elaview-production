...
const SpaceMarker: React.FC<{
  ...
  ({ space, isSelected, isInCart, onClick }) => {
    ...
    return (
      ...
      >
        <div
          className="flex cursor-pointer items-center justify-center transition-all duration-200 hover:scale-110"
          style={{
            width: `${size}px`,
            height: `${size}px`,
            borderRadius: "50%",
...
const MyLocationButton: React.FC<{
  ...
  return (
    <button
      onClick={onClick}
      disabled={isLocating}
      className="absolute top-4 right-4 z-10 rounded-lg bg-white p-3 shadow-lg transition-all duration-200 hover:bg-gray-50 hover:shadow-xl disabled:cursor-not-allowed disabled:bg-gray-100"
...
export const GoogleMap: React.FC<GoogleMapProps> = ({
  ...
  const { points } = useGoogleMap({ spaces, onBoundsChange });
  ...
  const { clusters, supercluster } = useSupercluster({
    points: space_points,
    bounds: [
      -117.14815159329673, // west
      29.38895491106978, // south
      -75.13305342959588, // east
      80.882634400652414, // north
    ],
    zoom: 20,
    options: {
  ...
  const legend = useMemo(
    () => (
      <div className="absolute bottom-8 left-8 z-10 hidden max-w-xs rounded-xl border border-slate-800 bg-slate-900/95 p-4 shadow-2xl backdrop-blur-xl md:block">
        <h4 className="mb-3 text-sm font-semibold text-white">Space Types</h4>
